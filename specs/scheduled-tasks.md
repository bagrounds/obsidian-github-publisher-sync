...
📋 For each task (direct library calls, no subprocesses):
   ...
   ├── 💑 blog-series:relationship-miniseries → check exists → pull → generate → image → sync
...
| 10 AM | `blog-series:relationship-miniseries` | 💑 Relationship Miniseries weekly science-fiction drama |
...
| `relationship-miniseries` | gemini-3.1-flash-lite-preview → gemini-3-flash-preview | `RELATIONSHIP_MINISERIES_PRIORITY_USER` |

🔒 chickie-loo, auto-blog-zero, and relationship-miniseries are restricted to Gemini 3+ models for highest quality blog posts.
