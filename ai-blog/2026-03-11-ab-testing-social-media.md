...
1. **Tags** via prompt A (same tags that an A post would get)
2. **Question** via prompt B (question-only prompt)
...
// gemini.ts — variant B: two calls, prompt A for tags + prompt B for question
if (variant === "B") {
  const [tags, question] = await Promise.all([
    callGemini(model, buildPromptForVariant("A", reflection)),
    callGemini(model, buildPromptForVariant("B", reflection)),
  ]);
