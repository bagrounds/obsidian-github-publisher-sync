...
const simulateFinalTranscript = (page: Page, transcript: string) =>
  page.evaluate(
    (text) => window.__wordMeter.simulateFinalTranscript(text),
    transcript,
  );
...
test.describe("Word Meter — PureScript build — slice 1 — recording", () => {
  test("renders the panel and identifies the build", async ({ page }) => {
    ...
    await expect(page.getByTestId("wm-version")).toHaveText(
      /word meter \(purescript\) v0\.1\.0/i,
    );
  });
  ...
  test("clicking the toggle flips listening status and button label", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
  ...
  test("injecting a final transcript while listening increments the count", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.getByTestId("wm-toggle").click();
  ...
  test("injecting a transcript while idle does not change the count", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await simulateFinalTranscript(page, "hello world");
...
test.describe("Word Meter — PureScript build — slice 2 — live captions", () => {
  test("renders an empty captions panel with a placeholder before any speech", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
  ...
  test("appends one caption per injected final transcript", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.getByTestId("wm-toggle").click();
...
test.describe("Word Meter — PureScript build — slice 4 — event log", () => {
  ...
  test("does not log anything while a counting session is still open", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.evaluate(() => {
  ...
  test("stop pushes one entry with started clock, duration, word count and wpm rate", async ({
    ...
    await expect(entry.getByTestId("wm-event-log-entry-duration")).toHaveText(
      "30s",
    );
    await expect(entry.getByTestId("wm-event-log-entry-words")).toHaveText(
      "3 w",
    );
    await expect(entry.getByTestId("wm-event-log-entry-rate")).toHaveText(
      "6.0 wpm",
    );
    const startedText =
  ...
  test("stop/restart appends a second entry in chronological order", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.evaluate(() => {
    ...
    await expect(
      entries.nth(0).getByTestId("wm-event-log-entry-duration"),
    ).toHaveText("1m 0s");
    await expect(
      entries.nth(0).getByTestId("wm-event-log-entry-words"),
    ).toHaveText("1 w");
    await expect(
      entries.nth(0).getByTestId("wm-event-log-entry-rate"),
    ).toHaveText("1.0 wpm");
    await expect(
      entries.nth(1).getByTestId("wm-event-log-entry-duration"),
    ).toHaveText("2m 0s");
    await expect(
      entries.nth(1).getByTestId("wm-event-log-entry-words"),
    ).toHaveText("3 w");
    await expect(
      entries.nth(1).getByTestId("wm-event-log-entry-rate"),
    ).toHaveText("1.5 wpm");
  });

  test("intervals with no recognized utterances log a zero-word session", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.evaluate(() => {
  ...
  test("caps the event log at the most recent counting sessions", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    ...
    const length = await page.evaluate(() =>
      window.__wordMeter.getEventLogLength(),
    );
...
test.describe("Word Meter — PureScript build — slice 3 — stats dashboard", () => {
  test("renders all five stat tiles starting at zero / em-dash", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
  ...
  test("captures the first-started timestamp on the very first start", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.evaluate(() => window.__wordMeter.startAt(1_700_000_000_000));
    const firstStartedAt = await page.evaluate(() =>
      window.__wordMeter.getFirstStartedAt(),
    );
    expect(firstStartedAt).toBe(1_700_000_000_000);
    await expect(page.getByTestId("wm-started")).not.toHaveText("—");
  });

  test("words / minute over the short window after a full minute", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.evaluate(() => {
    ...
    expect(await page.evaluate(() => window.__wordMeter.getRateShort())).toBe(
      6,
    );
    await expect(page.getByTestId("wm-rate-short")).toHaveText("6.0");
  });

  test("duration tile reflects active listening time across stop / start", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.evaluate(() => {
    ...
    expect(await page.evaluate(() => window.__wordMeter.getDurationMs())).toBe(
      60_000,
    );
    await expect(page.getByTestId("wm-duration")).toHaveText("1m 0s");
  });

  test("overall words / minute uses active listening time, not wall clock", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    ...
    expect(await page.evaluate(() => window.__wordMeter.getRateOverall())).toBe(
      3,
    );
    await expect(page.getByTestId("wm-rate-overall")).toHaveText("3.0");
  });

  test("long-window rate counts everything inside the trailing 10 minutes", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.evaluate(() => {
...
test.describe("Word Meter — PureScript build — slice 5 — diagnostics", () => {
  test("renders the collapsible drawer collapsed by default with the snapshot in its content", async ({
    ...
    await expect(page.getByTestId("wm-diagnostics-toggle")).toHaveText(
      /diagnostics/i,
    );
    ...
    await expect(page.getByTestId("wm-diagnostics-content")).toContainText(
      "version",
    );
    await expect(page.getByTestId("wm-diagnostics-content")).toContainText(
      "0.1.0",
  ...
  test("clicking the summary expands the drawer", async ({ page }) => {
    ...
    await expect(page.getByTestId("wm-diagnostics")).toHaveAttribute(
      "open",
      "",
    );
  });
  ...
  test("getDiagnosticsDrawerOpen reflects open/closed drawer state", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    expect(
      await page.evaluate(() => window.__wordMeter.getDiagnosticsDrawerOpen()),
    ).toBe(false);
  ...
  test("records the init event in the diagnostics log on startup", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    expect(
      await page.evaluate(() => window.__wordMeter.getDiagnosticsLength()),
    ).toBeGreaterThan(0);
    await expect(page.getByTestId("wm-diagnostics-content")).toContainText(
      "init",
    );
  ...
  test("records start, transcript, and stop entries through the reducer", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
...
test.describe("Word Meter — PureScript build — slice 6 — reset + persistence", () => {
  ...
  test("reset button asks for confirmation and wipes accumulated stats on accept", async ({
    ...
    await expect(page.getByTestId("wm-event-log-placeholder")).toBeVisible();
    await expect(page.getByTestId("wm-event-log-entry")).toHaveCount(0);
    expect(await page.evaluate(() => window.__wordMeter.getTotalWords())).toBe(
      0,
    );
  });

  test("declining the confirmation leaves stats untouched", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.evaluate(() => {
  ...
  test("totals and event log survive a full page reload via localStorage", async ({
    ...
    const persistedKey = await page.evaluate(() =>
      Object.keys(window.localStorage).find((k) =>
        k.startsWith("word-meter-ps:state"),
      ),
    );
    ...
    expect(
      await page.evaluate(() => window.__wordMeter.getEventLogLength()),
    ).toBe(2);
  });
...
test.describe("Word Meter — PureScript build — slice 7 — keep-awake toggle", () => {
  test("renders a keep-awake checkbox that defaults to checked", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    ...
    expect(await page.evaluate(() => window.__wordMeter.getKeepAwake())).toBe(
      true,
    );
  ...
  test("stopping releases the wake lock and clears the status", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.getByTestId("wm-toggle").click();
    ...
    await page.getByTestId("wm-toggle").click();
    expect(
      await page.evaluate(() => window.__wordMeter.getWakeLockHeld()),
    ).toBe(false);
    await expect(page.getByTestId("wm-keep-awake-status")).toHaveText("");
    ...
    expect(/wake lock (acquired|failure|release)/i.test(diagnostics)).toBe(
      true,
    );
  });

  test("starting with keep-awake off does not request a wake lock", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.evaluate(() => window.__wordMeter.setKeepAwake(false));
    ...
    expect(/wake lock acquired|wake lock failure/i.test(diagnostics)).toBe(
      false,
    );
  });
...
test.describe("Word Meter — PureScript build — slice 8 — recognition errors", () => {
  ...
  test("an unknown error code falls back to a generic banner", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.getByTestId("wm-toggle").click();
    ...
    await expect(page.getByTestId("wm-error")).toHaveText(
      "Recognition error: weird",
    );
  });

  test("an empty error code renders the unknown-code banner", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.getByTestId("wm-toggle").click();
    ...
    await expect(page.getByTestId("wm-error")).toHaveText(
      "Recognition error: unknown",
    );
  });

  test("starting again after a network error clears the banner", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.getByTestId("wm-toggle").click();
  ...
  test("getErrorBanner reflects the rendered text", async ({ page }) => {
    ...
    expect(
      await page.evaluate(() => window.__wordMeter.getErrorBanner()),
...
test.describe("Word Meter — PureScript build — slice 9c — language-not-supported retry", () => {
  test("on-device language-not-supported swaps to cloud once without showing a banner", async ({
    ...
    expect(
      /language-not-supported at runtime — falling back to cloud/.test(
        diagnostics,
      ),
    ).toBe(true);
...
test.describe("Word Meter — PureScript build — slice 9b — on-device pre-flight", () => {
  test("with on-device pre-flight disabled, start logs the cloud-fallback diagnostic and never sets the download status", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.getByTestId("wm-toggle").click();
    ...
    expect(
      /on-device API absent — falling back to cloud/.test(diagnostics),
    ).toBe(true);
  ...
  test("getRecognitionStatusOverride defaults to empty on idle", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    expect(
...
test.describe("Word Meter — PureScript build — vdom scroll preservation", () => {
  test("scrolled diagnostics drawer keeps its scroll position across a rerender", async ({
    ...
    // Open the drawer so the <pre> participates in the layout and can be
    // scrolled. The drawer is collapsed by default.
    await page.getByTestId("wm-diagnostics-toggle").click();
    await expect(page.getByTestId("wm-diagnostics")).toHaveAttribute(
      "open",
      "",
    ...
    // Fill the diagnostics log with many entries so the rendered <pre>
    // overflows its max-height and exposes a real scrollbar.
    await page.evaluate(() => {
      const capacity = window.__wordMeter.getDiagnosticsLimit();
      const iterations = Math.max(capacity, 200);
      for (let i = 0; i < iterations; i++) {
        const startTs = i * 1_000;
        window.__wordMeter.startAt(startTs);
        window.__wordMeter.simulateFinalTranscriptAt(
    ...
    const diagnosticsContent = page.getByTestId("wm-diagnostics-content");
    const maxScrollTop: number = await diagnosticsContent.evaluate(
      (element: HTMLElement) => element.scrollHeight - element.clientHeight,
    );
    expect(maxScrollTop).toBeGreaterThan(0);

    // Scroll roughly to the middle of the overflow region.
    const targetScrollTop = Math.floor(maxScrollTop / 2);
    await diagnosticsContent.evaluate((element: HTMLElement, top: number) => {
      element.scrollTop = top;
    }, targetScrollTop);
    const scrollAfterScrolling: number = await diagnosticsContent.evaluate(
      (element: HTMLElement) => element.scrollTop,
    );
    expect(scrollAfterScrolling).toBeGreaterThan(0);

    // Trigger another reducer dispatch so Vdom.mount runs a fresh render.
    await page.evaluate(() => window.__wordMeter.tick(999_999_000));

    const scrollAfterRerender: number = await diagnosticsContent.evaluate(
      (element: HTMLElement) => element.scrollTop,
    );
    // The renderer rebuilds the DOM on every dispatch, so without scroll
