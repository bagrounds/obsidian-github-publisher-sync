...
2. **Unfocused imagery** — Even when the prompt fit, a wall of Markdown about CI pipelines and YAML workflows isn't a great image prompt. The model struggled to extract a coherent visual concept.
...
The `describeImageWithGemini` function sends the blog content to `gemini-2.5-flash` with a system prompt requesting a concise visual description under 150 words, with no text in the image. The result is a tight, evocative image prompt that Flux can work with.

When `GEMINI_API_KEY` is available, the describer is automatically created by `resolveImageProvider` and threaded through to `processNote` and `backfillImages`. When unavailable, the fallback is the original `buildImagePrompt` (full content passthrough).
...
| `image_model` | Model identifier (e.g., `@cf/black-forest-labs/flux-1-schnell`) | Reproducibility |
| `image_prompt` | The prompt sent to the image model | Debugging and prompt iteration |

A new `updateFrontmatterFields` function handles this generically — it creates, updates, or preserves existing frontmatter fields, with proper YAML quoting for values containing special characters like `@`, `:`, or quotes.
...
When the pipeline encounters `regenerate_image: true`:

1. **Removes** the existing image embed from the post
...
| `@cf/leonardo/lucid-origin` | ~660+ | ~15 | Partner | Leonardo's most prompt-responsive model, great text rendering |
| `@cf/leonardo/phoenix-1.0` | ~550+ | ~18 | Partner | Strong prompt adherence and coherent text |

**Verdict**: Impressive quality, but 6–18 images/day is too constrained for batch backfill. The multipart API format also requires code changes.
...
- **Excellent prompt adherence**: Handles complex scenes and compositional prompts well.
- **230 free images/day**: More than enough for daily backfill of all blog series.
- **Fast**: 4 diffusion steps, near-instant generation.
...
|-------|--------------|-----------------|------------|-------|
| `@cf/black-forest-labs/flux-2-klein-4b` | ~31 (512×512) | ~320 | Multipart | Distilled 4B, fast but multipart API |
...
**Stick with `@cf/black-forest-labs/flux-1-schnell`.** It offers the best balance of:
- Image quality (Flux 1 architecture, 12B parameters)
...
125 tests pass (up from 92). New test coverage includes:
...
- **`removeImageEmbed`** — Obsidian wiki syntax removal, prefix handling, newline collapse
...
All 33 new tests plus all 92 existing tests pass.
