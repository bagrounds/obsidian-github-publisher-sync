...
test.describe("Word Meter — PureScript build — slice 3 — stats dashboard", () => {
  ...
  test("overall words / minute uses active listening time, not wall clock", async ({
    ...
    await loadWordMeter(page, "ps");
    // 6 words spoken over a 60s listening window with a 60s paused gap in
    // the middle. Overall WPM divides by active time (120s) → 3 wpm,
    // and ignores the paused 60s.
    await page.evaluate(() => {
      window.__wordMeter.startAt(0);
...
test.describe("Word Meter — PureScript build — slice 2 — live captions", () => {
  ...
  test("drops empty transcripts and keeps only the most recent six entries", async ({
    page,
  }) => {
    await loadWordMeter(page, "ps");
    await page.getByTestId("wm-toggle").click();
