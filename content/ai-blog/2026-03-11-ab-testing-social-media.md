...
tags:
---
[Home](../index.md) > [AI Blog](./index.md) | [⏮️ 2026-03-10 | 🔊 Teaching the Robot to Breathe 🤖](./2026-03-10-tts-semicolon-injection.md) [⏭️ 2026-03-11 | 🏗️ From GitLab to GitHub — Migrating a PureScript Deck-Building Game 🤖](./2026-03-11-domination-gitlab-to-github-migration.md)  
# 2026-03-11 | 🧪 AB Testing the Robot's Voice - Prompt Experiments for Social Media Engagement 🤖  
...
- ⚙️ **Customizable algorithmic feeds** amplify content that generates early engagement  
...
1. 🏷️ **Tags** via prompt A → Gemma (`gemma-3-27b-it`) - smaller, faster, sufficient for tag generation  
...
1. ⏱️ **Parsing the server's retry delay** from the error details (e.g. `retryDelay: "14s"`)  
2. ⏲️ **Waiting the specified duration** before retrying  
...
1. 🏷️ **Remove topic tags** from right to left  
2. 📄 **Remove entire topic line** (and preceding blank line)  
3. ✂️ **Strip subtitle from title** - remove after the first colon (e.g. Prediction Machines: The Simple Economics of AI → Prediction Machines). The title appears in the URL preview anyway.  
...
⚙️ The environment variable `AB_TEST_VARIANT` overrides random selection for manual testing (forces all platforms to the same variant):  
...
- 🛠️ `PostAssembler`: `(ModelOutput, ReflectionData) → PostText` (how to assemble the final post)  
...
1. ✅ **~~📊 Automated experiment log collection~~** - ✅ Records are now auto-persisted to the vault's `data/ab-test/` directory and analyzed incrementally on every pipeline run.  
...
5. 🖼️ **Visual content experiments** - 🖼️ Test whether including different OG image styles (thumbnails, illustrations, text cards) affects engagement.  
...
*⏱️ Two hours later, it flipped again. Tails. It asked a question.*  
