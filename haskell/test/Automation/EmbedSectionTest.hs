...
tests = testGroup "EmbedSection"
  ...
  , testCase "section headers use correct emojis" $ do
      assertBool "tweet has bird" $ T.isInfixOf "🐦" tweetSectionHeader
      assertBool "bluesky has butterfly" $ T.isInfixOf "🦋" blueskySectionHeader
      assertBool "mastodon has elephant" $ T.isInfixOf "🐘" mastodonSectionHeader
