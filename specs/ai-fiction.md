...
- **Model pool**: `gemini-2.5-flash`, `gemini-2.5-flash-lite`, `gemini-3.1-flash-lite-preview`, `gemini-3-flash-preview`, `gemini-2.0-flash`, `gemma-4-31b-it`, `gemma-4-26b-a4b-it`, `gemma-3-27b-it`. Authoritative model documentation lives at [the Gemini API model index](https://ai.google.dev/gemini-api/docs/models), [Gemini API deprecations](https://ai.google.dev/gemini-api/docs/deprecations), and [Gemma on the Gemini API](https://ai.google.dev/gemma/docs/core/gemma_on_gemini_api). Each model constructor in `Automation.Gemini` carries an inline doc URL. The `test-fiction-models` executable (and the matching `test-fiction-models.yml` workflow, triggerable from the GitHub mobile web UI) verifies every model in the pool against the live Gemini API on demand.
