...
getAccessTokenWithScope jwtScope manager serviceAccountKey = do
  ...
  case parseRSAPrivateKey (privateKey serviceAccountKey) of
    ...
    Right rsaPrivateKey -> do
      ...
      case createJwt claims rsaPrivateKey of
        Left err -> pure $ Left err
        Right jwt -> do
          ...
          case status of
            ...
            httpCode -> pure $ Left $
              "Token endpoint returned status " <> T.pack (show httpCode)
