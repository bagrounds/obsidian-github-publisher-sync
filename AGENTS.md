...
- 🪪 No `Impl` / `_impl` suffix and no jargon decorators on identifiers: never name something `fooImpl`, `fooInternal`, `fooHelper`, `fooRaw`, or `fooUnsafe` just because it is an FFI binding, a lower-level helper, or the inside of a wrapper. The job of an identifier is to name the thing; the type system, the module boundary, and the language's `foreign import` keyword already say what kind of thing it is. If a wrapper genuinely adds behavior, give the wrapper a different concept-level name (for example `currentTimeMillis` over `nowMs`, not `nowMsImpl` and `nowMs`). This rule applies to every language in the repository — PureScript foreign imports, TypeScript helpers, Haskell internals, JavaScript exports.
