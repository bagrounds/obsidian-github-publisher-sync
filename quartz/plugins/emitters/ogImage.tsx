...
async function loadManifest(): Promise<OgCacheManifest> {
  try {
    ...
    console.log(
      chalk.cyan(
        `[OG Cache] Loaded manifest with ${Object.keys(manifest).length} entries`,
      ),
    );
    return manifest;
  } catch (err) {
    console.log(
      chalk.yellow("[OG Cache] No existing manifest found, creating new one"),
    );
    return {};
  }
}
...
async function saveManifest(manifest: OgCacheManifest): Promise<void> {
  try {
    ...
    console.log(
      chalk.green(
        `[OG Cache] Saved manifest with ${Object.keys(manifest).length} entries`,
      ),
    );
  } catch (err) {
...
async function shouldSkipGeneration(
  ...
  if (copied) {
    ...
    console.log(
      chalk.yellow(`[OG Cache] Cache miss (file not found) for ${slug}`),
    );
    return false;
  }
}
...
async function processOgImage(
  ...
  try {
    ...
    console.warn(
      chalk.yellow(`[OG Cache] Failed to cache image for ${slug}: ${err}`),
    );
  }
