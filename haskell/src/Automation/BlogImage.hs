...
generateAndSaveImage manager provider notePath attachmentsDir content baseName = do
  ...
  case promptResult of
    ...
    Right prompt -> do
      imageResult <- ipcGenerator provider manager (unApiKey (ipcApiKey provider)) (ipcModel provider) prompt
      case imageResult of
        Left err -> do
...
resolvePrompt manager provider content =
  case extractFrontmatterValue content "image_prompt" of
    ...
    _ -> case ipcDescribePrompt provider of
      Just describer -> do
        result <- describer manager (unApiKey (ipcApiKey provider)) (ipcModel provider) content
