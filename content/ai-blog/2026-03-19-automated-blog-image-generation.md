...
The design follows the repository's established patterns: pure functions with dependency injection, JSON-structured logging, and strong typing.  
...
The backfill crawler processes directories from newest to oldest. When it inserts an image into a post, it updates the `updated` frontmatter timestamp for every file in the chain from the latest post back to the insertion point. This creates an unbroken BFS trail that Obsidian's Enveloppe plugin follows when syncing.  
  
Key safety rails:  
- **Skips future reflections** — Posts dated after today are still being written  
