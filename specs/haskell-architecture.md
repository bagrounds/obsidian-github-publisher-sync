...
- [x] ~~Extract `Automation.InternalLinking.Gemini`~~ — Dissolved. Gemini is an orthogonal concern to InternalLinking, so prompt construction (`buildIdentificationPrompt`), API calls with retry (`identifyBooksWithGemini`), and response parsing (`parseGeminiBookPaths`) live directly in `InternalLinking.hs` as orchestration code that uses `Automation.Gemini`.
- [x] Slim `Automation.InternalLinking` from 942 to 425 lines (55% reduction) — orchestration plus Gemini interaction: `FileResult`/`LinkingResult` types (with full field names), constants (`defaultLinkingModel`, `indexableDirs`, `traversableDirs`), `extractBody`, `alreadyAnalyzed`, `applyReplacements`, `buildIdentificationPrompt`, `identifyBooksWithGemini`, frontmatter updates, file processing (`processFile`), and top-level orchestration (`run`, `processFiles`). No re-exports — consumers import directly from the sub-module that defines the symbol.
- [x] 102 new tests (1597 → 1701): `MaskingTest` (30 tests: frontmatter, fenced code, inline code, markdown links, wikilinks, headings, URLs, bold, composition, properties), `LinkExtractionTest` (24 tests: path normalization, relative paths, split/join, suffix checking, link extraction, deduplication, properties), `CandidateDiscoveryTest` (48 tests: constants, emoji stripping, regex escaping, wikilinks, context, title extraction, candidate finding, properties). Prompt-building tests merged into `InternalLinkingTest`.
...
38. **Gemini is an orthogonal concern**: When a feature calls the Gemini API, the prompt construction, retry logic, and response parsing are part of the feature's orchestration — not a separate "Gemini integration" module. The `Automation.Gemini` module provides the API client; each feature module calls it directly. Creating `Feature.Gemini` submodules conflates two orthogonal domains.
